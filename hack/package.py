...
config = os.path.join(script_home,"..","deploy")
...
# load defaults 
with open(os.path.join(config,"defaults.yaml"), 'r') as stream:
...
shutil.copyfile(os.path.join(config,ns),os.path.join(releases,new_name))
ix += 1
...
shutil.copyfile(os.path.join(config,sa),os.path.join(releases,new_name))
ix += 1

# copy rbac_role
rbac_role_file = "role.yaml"
new_name = "%03d_%s" % (ix,rbac_role_file)
# load rbac_role from kubebuilder
with open(os.path.join(config,rbac_role_file), 'r') as rbacstream:
    rbac_role = yaml.safe_load(rbacstream)
# rename
rbac_role['metadata']['name'] =  "ibmcloud-iam-operator-manager-role"  
...
shutil.copyfile(os.path.join(config,rb),os.path.join(releases,new_name))
ix += 1

# copy deployment file
deploy_file = "operator.yaml"
new_name = "%03d_%s" % (ix,deploy_file)
# load rbac_role from kubebuilder
with open(os.path.join(config,deploy_file), 'r') as deploystream:
...
with open(os.path.join(config,"template.package.yaml"), 'r') as stream:
    ...
    with open(os.path.join(olm,"..","ibmcloud_iam_operator.package.yaml"), "w") as outfile:
        yaml.dump(pkg, outfile, default_flow_style=False)
 
# fill in cluster service version from template, deployment and roles
with open(os.path.join(config,"template.clusterserviceversion.yaml"), 'r') as stream:
    ...
    with open(os.path.join(olm,"ibmcloud_iam_operator."+args.version+".clusterserviceversion.yaml"), "w") as outfile:
