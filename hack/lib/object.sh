...
function object::wait_operator_ready() {
    ...
    until [ "$(kubectl -n ibmcloud-iam-operators get po | grep 'Running' | awk '{print $3}')" == "Running" ]; do
        printf "."
        sleep 2
    done
    printf $CHECKMARK
...
function object::wait_accesspolicy_online() {
    ...
    object::wait_resource_online "accesspolicy.ibmcloud.ibm.com" $name $retries
}

# wait for accessgroup to be online
function object::wait_accessgroup_online() {
    local name="$1"
    local retries="$2"

    object::wait_resource_online "accessgroup.ibmcloud.ibm.com" $name $retries
}

# wait for customrole to be online
function object::wait_customrole_online() {
    local name="$1"
    local retries="$2"

    object::wait_resource_online "customrole.ibmcloud.ibm.com" $name $retries
}
